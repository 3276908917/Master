...
=
(2 \pi)^3 \delta_D (\mathbm{k} - \mathbm{k}') P(k)
\end{equation}
...
\textit{linear-theory} power spectra $P_\text{L}(k)$. This means that we
are including only linear terms in a perturbative expansion of $\delta (x)$.
This entails that we neglect all but the dominant energy species' impact on
...
%s What is the problem with parameters that involve $h$?
