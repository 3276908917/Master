...
enough scales, the Universe looks statistically homogeneous and isotropic
(\cbib{Ntelis}). That is, there are neither 
...
of as the excess probability that some galaxy will be located at a distance of 
...
linear-theory power spectra of non-neutrino matter.
\footnote{\textcolor{red}{Andrea added the following comment: ``We should 
...
For example, they can be used to predict non-linear power spectra, like in 
...
counterpart, so the linear power spectrum is still necessary to evaluate the
non-linear power spectrum (A. G. S\'{a}nchez, private communication, 2022).
...
undergone two major periods of domination (the Universe having only relatively
recently entered a third period, dominated by dark energy): first by 
radiation, then by matter. The power spectrum displays this history. The
...
two parts, depending 
on when a Fourier mode enters the horizon
...
convention also persists in emulator literature (\cbib{Arico} and
\cbib{Mancini}).
...
\begin{figure}[htb]
    ...
    \end{subfigure}
        \centering
    \caption{Non-neutrino matter power spectra at $z=0$ for three different
    		models based on model zero (refer to table~\ref{tab: Aletheia_m0} 
    		for details) and varying only in the value of $h$. These two plots
...
use of $h$ units in observational cosmology. We illustrate here in
figure~\ref{fig: h_unit_Pk} one significant problem related to the
unstable meaning of $\sigma_8$: changes in $h$ change not only the power 
