...
on a single Boltzmann solver. As of today, the two As the researchers in our group have much more
experience with CAMB, we elected to use CAMB for our paper.

In principal, to maximize the generality of the
results introduced in this work, we would demonstrate their compatibility with
different Boltzmann solvers. In practice, the Boltzmann codes have already
been shown to be in excellent agreement with each other (\cbib{Seljak},
\cbib{Lesgourges}). However, later in this work, we will discover an
independent reason to consider alternate 

In the future, of course, it will be beneficial
to 
...
\ref{fig: n_s_dependence}, and \ref{fig: omega_nu_dependence}. It should be
