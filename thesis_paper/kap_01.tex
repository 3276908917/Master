...
\cbib{Finkbeiner}. \selfcomment{Redshift range?}
...
barotropic fluids with a constant of proportionality $\eulw$ between the
pressure $p$ and the density $\rho$ of the fluid, i.e. $p = \eulw \rho$.
$\eulw$ is
referred to as the equation of state parameter and in most cases is a
well-known constant; for radiation, $\eulw_\gamma = 1/3$, while for matter,
$\eulw_m = 0$. In
...
\begin{equation}
\eulw_\text{DE} = \eulw_0 + \eulw_a (1 - a)
\end{equation}

These $\eulw_0$ and $\eulw_a$ are cosmological parameters which also appear in
our statistical analyses, although our observations so far appear to support
a cosmological constant solution: $\eulw_0 = -1$ and $\eulw_a$ = 0. In the
first Friedmann equation, this form of dark energy would appear as an added
constant:
...
conservation \selfcomment{(Look up a derivation)} to find that

\begin{equation}
\label{eq: scale_factor_to_density}
\rho_i \propto a^{-3 (1 + \eulw_i)}
\end{equation}
...
\begin{equation}
\rho = \sum_i \frac{\rho_{i, 0}}{a^{3(1 + \eulw_i)}}
\end{equation}
...
\frac{8 \pi G}{3}
\,
\sum_i \frac{\rho_{i, 0}}{a^{3(1 + \eulw_i)}} - \frac{K}{a^2}
\end{equation}
...
$P_\text{L}(k)$, which is obtained when
$\delta \ll 1$ and only linear terms in 
the perturbative expansion of $\delta(\bm{x})$ are taken into account. 
\selfcomment{Consequently, we neglect all but the dominant energy species' 
impact on
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=\textwidth]{intro/m0_Pk}
...
confusion. Using instead $\sigma_{12}$, it is easier to assess the consistency
of different constraints, such as with the $\sigma_8$.
...
parameters is their incompatibility with evolution mapping. Consider that
$h = \sqrt{\sum \omega_i}$ is a mixed parameter because it combines the shape
parameters $\omega_b$ and $\omega_c$ with the evolution parameters $\omega_K$
and $\omega_\text{DE}$. Consequently, the $\Omega_i$ parameters are mixed,
because they depend on $h$. Mixed parameters violate a distinction that we
rely on in order to simplify the parameter space, so they must be excluded
from applications of evolution mapping. Although $\sigma_8$ behaves as an
evolution parameter for a fixed $h$, a varying $h$ will cause the meaning of
$\sigma_8$ to change, and so we cannot rely on it to characterize the amplitude
...
work. As of today, the two Boltzmann codes
...
the largest suites of features and are also actively developed.
...
elected to use CAMB as a starting point for our Python package CL. In 
principal, to maximize the generality of the results introduced in this 
...
inquiry for this work. However, later, in
section~\ref{sec: generate_emu_data}, we will concede an important, 
independent reason to consider use of CLASS, which becomes a promising route
for continuation of this work (see chapter~\ref{chap: conclusion}).
...
model 0 (tab~\ref{tab: Aletheia_m0}) as a middle ground and to create two
copies of model 0, with each
...
$\omega_\nu = 0$ is already at a hard boundary.
...
However, in the case of massless neutrinos, $A_s$ and $\sigma_{12}$ are
completely degenerate. Therefore, we do not include an analogous plot for
$\sigma_{12}$ (the other reason being that CAMB does not accept $\sigma_{12}$
as an input--see section~\ref{sec: generate_emu_data}).

The remaining parameters $\sigma_{12}$, $A_s$, and $\omega_\text{DE}$, as well
as the redshift $z$, shift only the amplitude of the power spectrum. We show
the amplitude shift associated with various $A_s$ values in figure~\ref{fig: 
A_s_dependence} and stress that the same plot only needs to be relabeled in 
order to illustrate the impact of $\sigma_{12}$, $z$, etc. 

\begin{figure}[ht!]
  \centering
  \includegraphics[width=\textwidth]{intro/parameter_demos/ombh2_impact_on_Pk}
  \caption[Impact of $\omega_b$ on $P(k)$]{Impact of $\omega_b$. Increased
  	...
  	in matter. However, this effect is subtle since the $x$-axis is
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=\textwidth]{intro/parameter_demos/omch2_impact_on_Pk}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=\textwidth]{intro/parameter_demos/n_s_impact_on_Pk}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=\textwidth]{intro/parameter_demos/A_s_impact_on_Pk}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=\textwidth]{intro/parameter_demos/omnuh2_impact_on_Pk}
...
neutrinos).\selfcomment{\footnote{\textcolor{red}{Andrea 
...
In the next chapter, we will consider modifications of evolution mapping in
order to accommodate massive neutrinos.
