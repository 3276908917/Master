...
\begin{figure}[htb]
  ...
  \caption[Impact of $\omega_b$ on $P(k)$]{Impact of $\omega_b$. Increased
  	density in baryons corresponds to a shift in power from larger to
  	smaller scales due to gravitational collapse. Higher densities also
  	smooth out the BAO. Recall in section~\ref{sec: Pk_intro} that
  	$\omega_b$ gives the turnover scale $k_s$ as well. This effect too can be
  	observed here, although it is quite subtle since the $x$-axis is
  	logarithmic and since even extreme values (relative to modern confidence
  	intervals) will only slightly shift $k_s$.} 
