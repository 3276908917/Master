...
Our code generates unit Latin hypercube samples which can be reused for many
different emulator setups. To use any given Latin hypercube in a different
emulator, one need only rescale the axes of the hypercube according to the
proper priors. CL accomplishes this through the use of prior files.

CL is packaged with three pairs of prior files.
Depending on which priors the 
user selects, the emulator will be a massive-neutrino or 
massless-neutrino emulator.

The $\omega_\nu$ prior is always $[0, 0.01]$ and the $\sigma_{12}$ prior is
always $[0.2, 1]$.
